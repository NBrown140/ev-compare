...
function EditableValue({
  ...
  return (
    <span
      ...
      className="cursor-text hover:text-on-surface hover:underline hover:decoration-dotted hover:underline-offset-2 transition-colors"
  ...
  return (
    <div className="flex flex-col gap-2 min-w-[220px]">
      <div className="flex items-baseline justify-between gap-2">
        <span
          className={`text-[13px] font-medium ${isActive ? "text-primary" : "text-outline"}`}
        >
          {label}
        </span>
        <span className="text-[11px] tabular-nums text-outline whitespace-nowrap flex items-center gap-0.5">
      ...
      >
        {/* Background track */}
        <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 h-1.5 rounded-full bg-surface-container" />

        {/* Active range fill */}
        <div
          className="absolute top-1/2 -translate-y-1/2 h-1.5 rounded-full bg-primary transition-[left,right] duration-75"
          style={{
            left: `${pct(lo)}%`,
            right: `${100 - pct(hi)}%`,
          }}
        ...
        <div
          ...
          className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-4.5 h-4.5 rounded-full bg-surface border-2 border-primary shadow-sm cursor-grab active:cursor-grabbing active:scale-110 transition-transform focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-1 outline-none"
...
function EnumSelect<T extends string>({
  ...
  return (
    <label className="flex flex-col text-sm">
      <span className="text-outline mb-1">{label}</span>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value as T | "all")}
        className="border border-outline-variant rounded-lg px-3 py-2 bg-surface text-on-surface text-sm"
      >
    ...
    <div className="rounded-xl border border-outline-variant bg-surface-container-low/60">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-outline-variant/60">
        <div className="flex items-center gap-2">
          ...
          {hasActiveFilters && (
            <span className="inline-flex items-center justify-center h-4 min-w-[16px] px-1 text-[10px] font-bold bg-primary text-on-primary rounded-full leading-none">
              {totalActiveCount}
            </span>
          )}
        </div>
      ...
      <div className="px-4 py-3">
        <div className="flex flex-wrap gap-x-5 gap-y-4 items-end">
          <fieldset className="flex flex-col text-sm">
            <span className="text-outline mb-1">
              Model Year
            </span>
            <div className="flex gap-1.5 flex-wrap">
              ...
              {filters.modelYears.length > 0 && (
                <button
                  ...
                  className="px-2.5 py-1.5 text-sm text-outline hover:text-on-surface"
                >
                  All
                </button>
              )}
          ...
          <div className="flex flex-col text-sm">
            <span className="text-outline mb-1">Segment</span>
            <FuzzyCombobox
        ...
        <div className="mt-3 flex items-center gap-2">
          <button
            ...
            className="text-xs font-medium text-on-surface bg-surface-container/70 hover:bg-surface-container px-2.5 py-1 rounded-md transition-colors flex items-center gap-1"
          >
            {showAdvanced ? "Less" : "More"}
            {!showAdvanced && advancedCount > 0 && (
              <span className="inline-flex items-center justify-center w-4 h-4 text-[10px] font-bold bg-primary text-on-primary rounded-full leading-none">
                {advancedCount}
              </span>
            )}
            <svg
          ...
          {hasActiveFilters && (
            <button
              onClick={onReset}
              className="text-xs text-outline hover:text-on-surface px-2 py-1 rounded-md hover:bg-surface-container/50 transition-colors"
            >
              Clear all
            </button>
          )}
